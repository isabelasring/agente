...
export default function OpenAIPanel({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hola, soy tu agente OpenAI. Analizo el documento activo y respondo con base en su contenido."
    }
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [lastLatencyMs, setLastLatencyMs] = useState<number | null>(null);
  const previousDocumentIdRef = useRef(selectedDocumentId);
  ...
  useEffect(() => {
    if (previousDocumentIdRef.current === selectedDocumentId) return;
    const previousLabel = formatDocumentLabel(previousDocumentIdRef.current);
    const nextLabel = formatDocumentLabel(selectedDocumentId);
    setMessages((prev) => [
      ...prev,
      {
        role: "assistant",
        content: `Documento activo cambiado: ${previousLabel} -> ${nextLabel}.`
      }
    ]);
  ...
  return (
    <div className={`phone-shell ${className}`.trim()}>
      ...
      <div className="messages">
        ...
        {loading && (
          <article className="message assistant typing" aria-live="polite" aria-label="OpenAI escribiendo">
            <span className="dot" />
            <span className="dot" />
            <span className="dot" />
          </article>
...
function formatDocumentLabel(documentId: string): string {
  if (!documentId || documentId === NO_DOCUMENT_SENTINEL) {
